...
app.http("phishing", {
  ...
  handler: async (request, context) => {
    ...
    async function dispatchMessage(message) {
      context.log(message);
      // Telegram first (preferred), Teams webhook as fallback for backward compat
      const sent = await sendTelegram(message, context);
      if (!sent && teams_webhook_url) {
        await fetch(teams_webhook_url, {
          ...
          .then((response) =>
            response.ok
              ? console.log("successfully dispatched MSG to Teams")
              : console.error(`Failed to dispatch to Teams: ${response.statusText}`)
          )
